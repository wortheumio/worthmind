"""Tight and reliable worth API client for worth indexer."""

from time import perf_counter as perf
from decimal import Decimal

from worth.utils.stats import Stats
from worth.utils.normalize import parse_amount, worth_amount, vests_amount
from worth.worth.http_client import HttpClient
from worth.worth.block.stream import BlockStream

class WorthClient:
    """Handles upstream calls to jussi/worths, with batching and retrying."""

    def __init__(self, url='https://api.wortheum.news', max_batch=50, max_workers=1):
        assert url, 'worth-API endpoint undefined'
    ...
    def gdgp_extended(self):
        ...
        unused = ['total_pow', 'num_pow_witnesses', 'confidential_supply',
                  'confidential_wbd_supply', 'total_reward_fund_worth',
        ...
        return {
            'dgpo': dgpo,
            'usd_per_worth': self._get_feed_price(),
            'wbd_per_worth': self._get_worth_price(),
            'worth_per_mvest': WorthClient._get_worth_per_mvest(dgpo)}

    @staticmethod
    def _get_worth_per_mvest(dgpo):
        worth = worth_amount(dgpo['total_vesting_fund_worth'])
        mvests = vests_amount(dgpo['total_vesting_shares']) / Decimal(1e6)
        return "%.6f" % (worth / mvests)
    ...
    def _get_feed_price(self):
        ...
        price = units['WBD'] / units['WORTH']
        return "%.6f" % price

    def _get_worth_price(self):
    ...
    def __exec_batch(self, method, params):
        ...
        Stats.log_worth(method, perf() - start, len(params))
        return result
