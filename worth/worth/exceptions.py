...
class RPCError(Exception):
    """Raised when an error is returned from upstream (jussi/worth)."""
    ...
    def is_recoverable(error):
        ...
        would be more appropriate but since worth uses only 'prepared
    ...
    @staticmethod
    def humanize(error):
        """Get friendly error string from worths RPC response."""
        ...
        if 'data' not in error: # eg db_lock_error
            name = 'error'
        elif 'name' in error['data']: # worths errs
...
class RPCErrorFatal(RPCError):
    """Represents a worths error which is not recoverable."""
    pass
