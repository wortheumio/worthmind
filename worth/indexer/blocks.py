...
from worth.indexer.custom_op import CustomOp
from worth.indexer.payments import Payments
...
class Blocks:
    """Processes blocks, dispatches work, manages `worth_blocks` table."""

    @classmethod
    def head_num(cls):
        """Get worth's head block number."""
        sql = "SELECT num FROM worth_blocks ORDER BY num DESC LIMIT 1"
        return DB.query_one(sql) or 0

    @classmethod
    def head_date(cls):
        """Get worth's head block date."""
        sql = "SELECT created_at FROM worth_blocks ORDER BY num DESC LIMIT 1"
    ...
    @classmethod
    def verify_head(cls, worth):
        """Perform a fork recovery check on startup."""
        worth_head = cls.head_num()
        if not worth_head:
            return

        # move backwards from head until worth/worth agree
        to_pop = []
        cursor = worth_head
        while True:
            assert worth_head - cursor < 25, "fork too deep"
            worth_block = cls._get(cursor)
            worth_hash = worth.get_block(cursor)['block_id']
            match = worth_block['hash'] == worth_hash
            log.info("[INIT] fork check. block %d: %s vs %s --- %s",
                     worth_block['num'], worth_block['hash'],
                     worth_hash, 'ok' if match else 'invalid')
            if match:
                break
            to_pop.append(worth_block)
            cursor -= 1

        if worth_head == cursor:
            return # no fork!

        log.error("[FORK] depth is %d; popping blocks %d - %d",
                  worth_head - cursor, cursor + 1, worth_head)

        # we should not attempt to recover from fork until it's safe
        fork_limit = worth.last_irreversible()
    ...
    def _get(cls, num):
        ...
        sql = """SELECT num, created_at date, hash
                 FROM worth_blocks WHERE num = :num LIMIT 1"""
        return dict(DB.query_row(sql, num=num))

    @classmethod
    def _push(cls, block):
        """Insert a row in `worth_blocks`."""
        num = int(block['block_id'][:8], base=16)
        txs = block['transactions']
        DB.query("INSERT INTO worth_blocks (num, hash, prev, txs, ops, created_at) "
    ...
    def _pop(cls, blocks):
        ...
        If consistency is critical, run worth with TRAIL_BLOCKS=-1 to only index
        ...
        As-is, there are a few caveats with the following strategy:
         ...
         - follow counts can get out of sync (worth needs to force-recount)
        ...
        for block in blocks:
            ...
            sql = "SELECT id FROM worth_posts WHERE created_at >= :date"
            post_ids = tuple(DB.query_col(sql, date=date))

            # remove all recent records -- communities
            DB.query("DELETE FROM worth_notifs        WHERE created_at >= :date", date=date)
            DB.query("DELETE FROM worth_subscriptions WHERE created_at >= :date", date=date)
            DB.query("DELETE FROM worth_roles         WHERE created_at >= :date", date=date)
            DB.query("DELETE FROM worth_communities   WHERE created_at >= :date", date=date)

            # remove all recent records -- core
            DB.query("DELETE FROM worth_feed_cache  WHERE created_at >= :date", date=date)
            DB.query("DELETE FROM worth_reblogs     WHERE created_at >= :date", date=date)
            DB.query("DELETE FROM worth_follows     WHERE created_at >= :date", date=date) #*

            # remove posts: core, tags, cache entries
            if post_ids:
                DB.query("DELETE FROM worth_posts_cache WHERE post_id IN :ids", ids=post_ids)
                DB.query("DELETE FROM worth_post_tags   WHERE post_id IN :ids", ids=post_ids)
                DB.query("DELETE FROM worth_posts       WHERE id      IN :ids", ids=post_ids)

            DB.query("DELETE FROM worth_payments    WHERE block_num = :num", num=num)
            DB.query("DELETE FROM worth_blocks      WHERE num = :num", num=num)
            DB.query("DELETE FROM worth_trxid_block_num WHERE block_num = :num", num=num)
    ...
    def save_trxids(cls, trxids):
        ...
        if data == '':
            return
        insert_sql = "INSERT INTO worth_trxid_block_num (trx_id, block_num) VALUES "
