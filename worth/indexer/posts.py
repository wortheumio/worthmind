...
class Posts:
    ...
    def last_id(cls):
        ...
        sql = "SELECT MAX(id) FROM worth_posts WHERE is_deleted = '0'"
    ...
    def get_id(cls, author, permlink):
        ...
        if url in cls._ids:
            ...
            cls._miss += 1
            sql = """SELECT id FROM worth_posts WHERE
    ...
    def get_id_and_depth(cls, author, permlink):
        ...
        if not _id:
            return (None, -1)
        depth = DB.query_one("SELECT depth FROM worth_posts WHERE id = :id", id=_id)
        return (_id, depth)

    @classmethod
    def is_pid_deleted(cls, pid):
        """Check if the state of post is deleted."""
        sql = "SELECT is_deleted FROM worth_posts WHERE id = :id"
    ...
    def insert(cls, op, date):
        ...
        sql = """INSERT INTO worth_posts (is_valid, is_muted, parent_id, author,
        ...
        sql += ";SELECT currval(pg_get_serial_sequence('worth_posts','id'))"
    ...
    def undelete(cls, op, date, pid):
        ...
        sql = """UPDATE worth_posts SET is_valid = :is_valid,
    ...
    def delete(cls, op):
        ...
        DB.query("UPDATE worth_posts SET is_deleted = '1' WHERE id = :id", id=pid)

        if not DbState.is_initial_sync():
            CachedPost.delete(pid, op['author'], op['permlink'])
            if depth == 0:
                # TODO: delete from worth_reblogs -- otherwise feed cache gets populated with deleted posts somwrimas
    ...
    def _get_parent_by_child_id(cls, child_id):
        ...
        sql = """SELECT id, author, permlink FROM worth_posts
                  WHERE id = (SELECT parent_id FROM worth_posts
                               WHERE id = :child_id)"""
