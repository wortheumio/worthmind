...
class Community:
    """Handles worth community registration and operations."""
    ...
    def register(cls, names, block_date):
        ...
        for name in names:
            #if not re.match(r'^worth-[123]\d{4,6}$', name):
            if not re.match(r'^worth-[1]\d{4,6}$', name):
                continue
            type_id = int(name[5])
            _id = Accounts.get_id(name)

            # insert community
            sql = """INSERT INTO worth_communities (id, name, type_id, created_at)
                          VALUES (:id, :name, :type_id, :date)"""
            DB.query(sql, id=_id, name=name, type_id=type_id, date=block_date)

            # insert owner
            sql = """INSERT INTO worth_roles (community_id, account_id, role_id, created_at)
    ...
    def validated_name(cls, name):
        ...
        if (name[:5] == 'worth-'
                and name[5] in ['1', '2', '3']
                and re.match(r'^worth-[123]\d{4,6}$', name)):
            return name
        return None
    ...
    def get_id(cls, name):
        ...
        if name in cls._ids:
            return cls._ids[name]
        sql = "SELECT id FROM worth_communities WHERE name = :name"
    ...
    def _get_name(cls, cid):
        ...
        sql = "SELECT name FROM worth_communities WHERE id = :id"
    ...
    def get_all_muted(cls, community_id):
        ...
        return DB.query_col("""SELECT name FROM worth_accounts
                                WHERE id IN (SELECT account_id FROM worth_roles
    ...
    def get_user_role(cls, community_id, account_id):
        ...
        return DB.query_one("""SELECT role_id FROM worth_roles
                                WHERE community_id = :community_id
...
class CommunityOp:
    ...
    def process(self):
        ...
        # Account-level actions
            ...
            DB.query("""INSERT INTO worth_roles
        ...
        # Post-level actions
            ...
            DB.query("""UPDATE worth_posts SET is_muted = '1'
            ...
            DB.query("""UPDATE worth_posts SET is_muted = '0'
                         WHERE id = :post_id""", **params)
            ...
            DB.query("""UPDATE worth_posts SET is_pinned = '1'
                         WHERE id = :post_id""", **params)
            self._notify('pin_post', payload=self.notes)
        elif action == 'unpinPost':
            DB.query("""UPDATE worth_posts SET is_pinned = '0'
                         WHERE id = :post_id""", **params)
    ...
    def _read_permlink(self):
        ...
        sql = """SELECT community_id FROM worth_posts WHERE id = :id LIMIT 1"""
    ...
    def _muted(self):
        ...
        sql = "SELECT is_muted FROM worth_posts WHERE id = :id"
        return bool(DB.query_one(sql, id=self.post_id))

    def _parent_muted(self):
        """Check parent post's muted status."""
        parent_id = "SELECT parent_id FROM worth_posts WHERE id = :id"
        sql = "SELECT is_muted FROM worth_posts WHERE id = (%s)" % parent_id
        return bool(DB.query_one(sql, id=self.post_id))

    def _pinned(self):
        """Check post's pinned status."""
        sql = "SELECT is_pinned FROM worth_posts WHERE id = :id"
        return bool(DB.query_one(sql, id=self.post_id))

    def _flagged(self):
        """Check user's flag status."""
        from worth.indexer.notify import NotifyType
        sql = """SELECT 1 FROM worth_notifs
                  WHERE community_id = :community_id
                    AND post_id = :post_id
                    AND type_id = :type_id
                    AND src_id = :src_id"""
