...
from worth.utils.post import post_basic, post_legacy, post_payout, post_stats, mentions
...
class CachedPost:
    """Maintain update queue and writing to `worth_posts_cache`."""
    ...
    def delete(cls, post_id, author, permlink):
        ...
        With worths, posts can be 'deleted' or unallocated in certain
        ...
        """
        DB.query("DELETE FROM worth_posts_cache WHERE post_id = :id", id=post_id)
        DB.query("DELETE FROM worth_post_tags   WHERE post_id = :id", id=post_id)
    ...
    def undelete(cls, post_id, author, permlink, category):
        ...
        'Undeletion' occurs when worth detects that a previously deleted
        author/permlink combination has been reused on a new post. Worth
        does not delete worth_posts entries because they are currently
        irreplaceable in case of a fork. Instead, we reuse the slot.
        It's important to immediately insert a placeholder in the cache
        table since worth only scans forward. This row's properties push
        it to the front of update-immediately queue.

        Alternate ways of handling undeletes:

         - delete row from worth_posts so that it can be re-indexed (re-id'd)
            - comes at a risk of losing expensive entry on fork (and no undo)
         - create undo table for worth_posts, worth_follows, etc, & link to block
         - rely on worths's post.id instead of database autoincrement
           - requires way to query worths post objects by id to be useful
             - batch get_content_by_ids in worths would be /huge/ speedup
    ...
    @classmethod
    def flush(cls, worth, trx=False, spread=1, full_total=None):
        ...
        cls._update_batch(worth, tuples, trx, full_total=full_total)
    ...
    def _select_paidout_tuples(cls, date):
        """Query worth_posts_cache for payout sweep.
        ...
        sql = """SELECT post_id FROM worth_posts_cache
        ...
        sql = """SELECT id, author, permlink
                 FROM worth_posts WHERE id IN :ids"""
    ...
    def _select_missing_tuples(cls, last_cached_id, limit=1000000):
        ...
        from worth.indexer.posts import Posts
        sql = """SELECT id, author, permlink, promoted FROM worth_posts
    ...
    def dirty_missing(cls, limit=250000):
        """Mark dirty all worth_posts records not yet written to cache."""
        from worth.indexer.posts import Posts
    ...
    @classmethod
    def _update_batch(cls, worth, tuples, trx=True, full_total=None):
        """Fetch, process, and write a batch of posts.

        Given a set of posts, fetch from worths and write them to the
        db. The `tuples` arg is the form of `[(url, id, level)*]`
        representing posts which are to be fetched from worths and
        updated in cache.

        Regarding _bump_last_id: there's a rare edge case when the last
        worth_post entry has been deleted "in the future" (ie, we haven't
        ...
        for tups in partition_all(1000, tuples):
            ...
            for pid, post, level in zip(post_ids, posts, post_levels):
                if post['author']:
                    ...
                    # worths simply returns a blank post  object w/ all
    ...
    def last_id(cls):
        ...
        if cls._last_id == -1:
            ...
            sql = "SELECT COALESCE(MAX(post_id), 0) FROM worth_posts_cache"
            cls._last_id = DB.query_one(sql)
        return cls._last_id

    @classmethod
    def _get_core_fields(cls, tups):
        """Cached posts must inherit some properties from worth_posts.

        Purpose
         - immutable `category` (returned from worths is subject to change)
    ...
    def _ensure_safe_gap(cls, last_id, next_id):
        ...
        sql = """SELECT COUNT(*) FROM worth_posts
    ...
    def _muted(cls, account, target):
        ...
        sql = """SELECT 1 FROM worth_follows
    ...
    def _tag_sqls(cls, pid, tags, diff=True):
        ...
        curr_tags = set()
        if diff:
            sql = "SELECT tag FROM worth_post_tags WHERE post_id = :id"
            curr_tags = set(DB.query_col(sql, id=pid))

        to_rem = (curr_tags - next_tags)
        if to_rem:
            sql = "DELETE FROM worth_post_tags WHERE post_id = :id AND tag IN :tags"
        ...
        if to_add:
            ...
            sql = "INSERT INTO worth_post_tags (post_id, tag) VALUES %s"
            sql += " ON CONFLICT DO NOTHING" # (conflicts due to collation)
            yield (sql % ','.join(vals), {'id': pid, **params})

    @classmethod
    def _insert(cls, values):
        return DB.build_insert('worth_posts_cache', values, pk='post_id')

    @classmethod
    def _update(cls, values):
        return DB.build_update('worth_posts_cache', values, pk='post_id')
