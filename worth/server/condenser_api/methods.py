"""Worths/condenser_api compatibility layer API methods."""

from functools import wraps

import worth.server.condenser_api.cursor as cursor
from worth.server.condenser_api.objects import load_posts, load_posts_reblogs
...
async def get_account_votes(context, account):
    ...
    raise ApiError("get_account_votes is no longer supported")
...
async def get_discussions_by_author_before_date(context, author: str = None, start_permlink: str = '',
    ...
    completely ignored in worths as well. This call is similar to
...
async def get_transaction(context, trx_id: str):
    ...
    """
    sql = "SELECT block_num FROM worth_trxid_block_num WHERE trx_id = :trx_id"
    block_num = await context['db'].query_row(sql, trx_id = trx_id)
    assert block_num != None, 'trx_id does not exist'
    block = context['worths'].get_block(num = block_num[0])
