...
import worth.server.bridge_api.cursor as cursor
from worth.server.bridge_api.objects import load_posts, load_posts_reblogs, load_profiles
from worth.server.common.helpers import (
    ...
    valid_tag,
    valid_limit)
from worth.server.worth_api.common import get_account_id
from worth.server.worth_api.objects import _follow_contexts
from worth.server.worth_api.community import list_top_communities
...
async def get_account_posts(context, sort, account, start_author='', start_permlink='',
    ...
    _id = await db.query_one("SELECT id FROM worth_posts_status WHERE author = :n", n=account)
    if _id:
        return []
