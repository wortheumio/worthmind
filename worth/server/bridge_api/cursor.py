...
async def _get_post_id(db, author, permlink):
    """Get post_id from worth db. (does NOT filter on is_deleted)"""
...
async def _get_community_id(db, name):
    """Get community id from worth db."""
    assert name, 'no comm name specified'
    _id = await db.query_one("SELECT id FROM worth_communities WHERE name = :n", n=name)
    return _id
...
async def pids_by_community(db, ids, sort, seek_id, limit):
    ...
    # hide posts
    #sql = "SELECT post_id FROM worth_posts_status WHERE list_type = '1'"
    #where.append("post_id NOT IN (%s)" % sql)

    # hide author
    sql = "SELECT author FROM worth_posts_status WHERE list_type = '3'"
...
async def pids_by_category(db, tag, sort, last_id, limit):
    ...
    if tag:
        if sort in ['payout', 'payout_comments']:
            ...
            sql = "SELECT post_id FROM worth_post_tags WHERE tag = :tag"
...
async def _pinned(db, community_id):
    ...
    sql = """SELECT id FROM worth_posts
              WHERE is_pinned = '1'
                AND is_deleted = '0'
...
async def _pids_by_type(db, list_type):
    ...
    sql = """SELECT post_id FROM worth_posts_status
...
async def pids_by_blog(db, account: str, start_author: str = '',
    ...
    skip = """
        SELECT id FROM worth_posts
         WHERE author = :account
           AND is_deleted = '0'
           AND depth = 0
           AND community_id IS NOT NULL
           AND id NOT IN (SELECT post_id FROM worth_reblogs
                           WHERE account = :account)"""

    # hide posts
    #hide = "SELECT post_id FROM worth_posts_status WHERE list_type = '1'"
    ...
    #    SELECT id
    #      FROM (
    #             SELECT id, author account, created_at FROM worth_posts
    #              WHERE depth = 0 AND is_deleted = '0' AND community_id IS NULL
    #              UNION ALL
    #             SELECT post_id id, account, created_at FROM worth_reblogs
    #           ) blog
...
async def pids_by_feed_with_reblog(db, account: str, start_author: str = '',
    ...
    if start_permlink:
        ...
        seek = """
          HAVING MIN(worth_feed_cache.created_at) <= (
            SELECT MIN(created_at) FROM worth_feed_cache WHERE post_id = :start_id
               AND account_id IN (SELECT following FROM worth_follows
    ...
    sql = """
        SELECT post_id, string_agg(name, ',') accounts
          FROM worth_feed_cache
          JOIN worth_follows ON account_id = worth_follows.following AND state IN (1,3)
          JOIN worth_accounts ON worth_follows.following = worth_accounts.id
         WHERE worth_follows.follower = :account
           AND worth_feed_cache.created_at > :cutoff
      GROUP BY post_id %s
      ORDER BY MIN(worth_feed_cache.created_at) DESC LIMIT :limit
    """ % seek
...
async def pids_by_replies(db, start_author: str, start_permlink: str = '',
    ...
    if start_permlink:
        sql = """
          SELECT parent.author,
                 child.id
            FROM worth_posts child
            JOIN worth_posts parent
    ...
    sql = """
       SELECT id FROM worth_posts
        WHERE parent_id IN (SELECT id FROM worth_posts
                             WHERE author = :parent
                               AND is_deleted = '0'
                          ORDER BY id DESC
                             LIMIT 10000) %s
...
async def pids_by_payout(db, account: str, start_author: str = '',
    ...
    if start_permlink:
        start_id = await _get_post_id(db, start_author, start_permlink)
        last = "(SELECT payout FROM worth_posts_cache WHERE post_id = :start_id)"
