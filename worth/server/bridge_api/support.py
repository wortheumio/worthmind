...
from worth.server.bridge_api.objects import _condenser_post_object
from worth.utils.post import post_to_internal
...
async def normalize_post(context, post):
    """Takes a worths post object and outputs bridge-api normalized version."""
    db = context['db']
    ...
    # load author
    sql = """SELECT id, reputation FROM worth_accounts WHERE name = :name"""
    ...
    promoted = wbd_amount(post['promoted']) if post['promoted'] != '0.000 WORTH' else None
