...
from worth.server.condenser_api import methods as condenser_api
from worth.server.condenser_api.tags import get_trending_tags as condenser_api_get_trending_tags
from worth.server.condenser_api.get_state import get_state as condenser_api_get_state
from worth.server.condenser_api.call import call as condenser_api_call
from worth.server.common.mutes import Mutes
from worth.server.common.payout_stats import PayoutStats

from worth.server.bridge_api import methods as bridge_api
from worth.server.bridge_api.thread import get_discussion as bridge_api_get_discussion
from worth.server.bridge_api.support import normalize_post as bridge_api_normalize_post
from worth.server.bridge_api.support import get_post_header as bridge_api_get_post_header
from worth.server.worth_api import community as worth_api_community
from worth.server.worth_api import notify as worth_api_notify
from worth.server.worth_api import stats as worth_api_stats

from worth.server.db import Db
...
def build_methods():
    """Register all supported worth_api/condenser_api.calls."""
    # pylint: disable=expression-not-assigned, line-too-long
    methods = Methods()

    methods.add(**{'worth.' + method.__name__: method for method in (
        db_head_state,
    )})
    ...
    methods.add(**{'bridge.' + method.__name__: method for method in (
        ...
        worth_api_notify.post_notifications,
        worth_api_notify.account_notifications,
        worth_api_notify.unread_notifications,
        worth_api_stats.get_payout_stats,
        worth_api_community.get_community,
        worth_api_community.get_community_context,
        worth_api_community.list_communities,
        worth_api_community.list_pop_communities,
        worth_api_community.list_community_roles,
        worth_api_community.list_subscribers,
        worth_api_community.list_all_subscriptions,
    )})
...
def run_server(conf):
    ...
    app['config']['worth.MAX_DB_ROW_RESULTS'] = 100000
    app['worths'] = conf.worth()
    #app['config']['worth.logger'] = logger
    ...
    async def head_age(request):
        """Get worth head block age in seconds. 500 status if age > 15s."""
        #pylint: disable=unused-argument
        healthy_age = 15 # worth is synced if head block within 15s
        try:
    ...
    async def health(request):
        """Get worth health state. 500 if db unavailable or too far behind."""
        #pylint: disable=unused-argument
    ...
    async def jsonrpc_handler(request):
        """Handles all worth jsonrpc API requests."""
