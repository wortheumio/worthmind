"""Worth API: account, post, and comment object retrieval"""
import logging
from worth.server.worth_api.common import get_account_id, estimated_wp
...
async def accounts_by_name(db, names, observer=None, lite=True):
    ...
    for row in rows:
        account = {
            ...
            'wp': int(estimated_wp(row['vote_weight'])),
            'rank': row['rank'],
...
async def _follow_contexts(db, accounts, observer_id, include_mute=False):
    sql = """SELECT following, state FROM worth_follows
...
async def comments_by_id(db, ids, observer=None):
    ...
    sql = """SELECT post_id, author, permlink, body, depth,
               ...
               FROM worth_posts_cache WHERE post_id IN :ids""" #votes
...
async def _reblogged_ids(db, observer, post_ids):
    sql = """SELECT post_id FROM worth_reblogs
