"""Worth API: Threaded discussion handling"""
import logging

from worth.server.worth_api.common import url_to_id, valid_comment_sort, valid_limit
from worth.server.worth_api.objects import comments_by_id
...
async def _fetch_children(db, root_id, start_id, sort, limit, observer=None):
    ...
    seek = ''
    if start_id:
        seek = """AND %s < (SELECT %s FROM worth_posts_cache
