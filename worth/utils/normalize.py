"""Methods to parse worths values and clean strings."""
...
def worth_amount(value):
    """Returns a decimal amount, asserting units are WORTH"""
    return parse_amount(value, 'WORTH')

def wbd_amount(value):
    """Returns a decimal amount, asserting units are WBD"""
    return parse_amount(value, 'WBD')

def parse_amount(value, expected_unit=None):
    """Parse worths-style amout/asset value, return (decimal, name)."""
...
def amount(string):
    """Parse a worths asset-amount as a Decimal(). Discard asset type."""
...
def legacy_amount(value):
    ...
    prec = {'WBD': 3, 'WORTH': 3, 'VESTS': 6}[asset]
    tmpl = ("%%.%df %%s" % prec)
...
def rep_log10(rep):
    """Convert raw worths rep into a UI-ready value centered at 25."""
    def _log10(string):
