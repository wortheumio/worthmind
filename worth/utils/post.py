"""Methods for normalizing worths post metadata."""
...
from worth.utils.normalize import wbd_amount, rep_log10, safe_img_url, parse_time, utc_timestamp
...
def post_to_internal(post, post_id, level='insert', promoted=None):
    """Given a worths post, build internal representation."""
...
def post_basic(post):
    ...
    if wbd_amount(post['max_accepted_payout']) == 0:
    ...
    # payout entirely in WP
    is_full_power = int(post['percent_worth_dollars']) == 0
...
def post_legacy(post):
    ...
    _legacy = ['id', 'url', 'root_comment', 'root_author', 'root_permlink',
               ...
               'max_accepted_payout', 'percent_worth_dollars',
...
def post_payout(post):
    ...
    payout = sum([
        wbd_amount(post['total_payout_value']),
        wbd_amount(post['curator_payout_value']),
        wbd_amount(post['pending_payout_value']),
    ])

    # `active_votes` was temporarily missing in dev -- ensure this condition
    # is caught ASAP. if no active_votes then rshares MUST be 0. ref: worth#2568
...
def post_stats(post):
    ...
    has_pending_payout = wbd_amount(post['pending_payout_value']) >= 0.02
