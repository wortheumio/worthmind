...
from worth.utils.system import colorize, peak_usage_mb
...
class WorthStats(StatsAbstract):
    """Tracks Worth client call timings."""
    ...
    def check_timing(self, call, ms, batch_size):
        ...
        par = self.PAR_WORTHS[call]
        over = per / par
        if over >= self.PAR_THRESHOLD:
            out = ("[WORTH][%dms] %s[%d] -- %.1fx par (%d/%d)"
...
class Stats:
    """Container for worths and db timing data."""
    PRINT_THRESH_MINS = 1
    ...
    @classmethod
    def log_worth(cls, method, secs, batch_size=1):
        """Log a worths call."""
        cls._worths.add(method, secs * 1000, batch_size)
        cls.add_secs(secs)
