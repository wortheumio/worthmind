...
from worth.utils.normalize import parse_time
from worth.worth.client import WorthClient

@pytest.fixture
def client():
    return WorthClient(url='https://api.wortheum.news')

def test_instance(client):
    assert isinstance(client, WorthClient)

def test_get_accounts(client):
    accounts = client.get_accounts(['wortheum', 'test-safari'])
    assert len(accounts) == 2
    assert accounts[0]['name'] == 'wortheum'
