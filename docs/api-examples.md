...
http -j post http://localhost:8080 jsonrpc=2.0 id=1 method=condenser_api.get_discussions_by_trending params:='{"start_author":"fredrikaa","start_permlink":"why-i-bought-my-brother-worth-for-christmas-and-how-you-can-do-the-same","tag":"","limit":2}'
...
http -j post http://localhost:8080 jsonrpc=2.0 id=1 method=condenser_api.get_discussions_by_feed params:='{"tag":"test-safari","limit":3,"start_author":"worthblog","start_permlink":"wortheum-winter-update-2017-reflection-our-vision-statement-and-mission-and-a-look-forward"}'
...
```
http -j post http://localhost:8080 jsonrpc=2.0 id=1 method=worth.db_head_state params:='{}'

http -j post http://localhost:8080 jsonrpc=2.0 id=1 method=worth.get_followers params:='{"account":"test-safari","skip":0,"limit":10}'
http -j post http://localhost:8080 jsonrpc=2.0 id=1 method=worth.get_following params:='{"account":"test-safari","skip":0,"limit":10}'

http -j post http://localhost:8080 jsonrpc=2.0 id=1 method=worth.get_follow_count params:='{"account":"test-safari"}'

http -j post http://localhost:8080 jsonrpc=2.0 id=1 method=worth.get_user_feed params:='{"account":"test-safari","skip":0,"limit":20,"ctx":"test-safari"}'
http -j post http://localhost:8080 jsonrpc=2.0 id=1 method=worth.get_blog_feed params:='{"account":"test-safari","skip":0,"limit":20,"ctx":"test-safari"}'

http -j post http://localhost:8080 jsonrpc=2.0 id=1 method=worth.get_related_posts params:='{"account":"test-safari","permlink":"tps-report-4-calm-before-the-storm"}'

http -j post http://localhost:8080 jsonrpc=2.0 id=1 method=worth.payouts_total
http -j post http://localhost:8080 jsonrpc=2.0 id=1 method=worth.payouts_last_24h

http -j post http://localhost:8080 jsonrpc=2.0 id=1 method=worth.search params:='{"query":"test-"}'
```


# worths.legacy (deprecated)

```
http -j post https://api.wortheum.news jsonrpc=2.0 id=1 method=call params:='["follow_api","get_follow_count",["test-safari"]]'
http -j post https://api.wortheum.news jsonrpc=2.0 id=1 method=call params:='["follow_api","get_followers",["test-safari","", "blog", 5]]'
http -j post https://api.wortheum.news jsonrpc=2.0 id=1 method=call params:='["follow_api","get_following",["test-safari","", "ignore", 2]]'

http -j post https://api.wortheum.news jsonrpc=2.0 id=1 method=call params:='["database_api","get_content",["test-safari", "34gfex-december-spam"]]'
```
